...
export declare type AnyFunction<P extends any[] = any, R = any> = (...parameters: P) => R;
/** Allow either A or B but not both at the same time */
export declare type Either<A, B> = Only<A, B> | Only<B, A>;
export declare type Entries<T> = Array<
	{
		[K in keyof T]: [keyof PickByValue<T, T[K]>, T[K]];
	}[keyof T]
>;
export declare type Filter<T, Validator> = T extends Validator ? T : never;
/** Allow autocompletion of union in addition to arbitrary values */
export declare type Flexible<Union extends T, T = string> = Union | (T & Record<never, never>);
/** Infers the return value of toJSON on the properties */
export declare type JSONState<T> = {
...
export declare type Only<U, V> = {
	...
	keyof U
>;
export declare type Overwrite<U, V> = Omit<U, keyof V> & V;
export declare type PickByValue<T, V> = Pick<
	T,
	{
...
export declare type Reverse<T extends any[]> = T extends [infer H, ...infer R]
	? [...Reverse<R>, H]
	: [];
...
export declare type Permutation<Union, Sliced = Union> = [Union] extends [never]
	? []
	: Union extends Union
 ...
 */
export declare type OptionalAnnex<T, Annex> = T extends {
	[P in keyof Annex]: {
		[K in P]: Annex[K];
	};
}[keyof Annex]
 ...
 */
export declare type PartialOmit<
	T,
 ...
 */
export declare type SingleProperty<T> = {
	[P in keyof T]: {
		[K in P]: T[P];
	} & {
